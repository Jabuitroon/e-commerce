...
interface CartProps {
  onCheckout?: () => void
  onViewBag?: () => void
  onCloseCart?: () => void // Para cerrar el desplegable si está dentro de un popover/modal
}

export function Cart({ onCheckout, onViewBag, onCloseCart }: CartProps) {
  const { cart, addToCart, decreaseQuantify, removeFromCart, clearCart } = useCart()
  ...
  if (!isAuth) {
    return (
      <div className="w-80 rounded-2xl bg-white p-6 shadow-xl border border-gray-100 text-center">
        <p className="text-sm font-medium text-gray-600">Inicia sesión para comenzar</p>
      </div>
    )
  }
    ...
    <div className="w-80 sm:w-96 rounded-2xl bg-white p-5 shadow-2xl border border-gray-100 transition-all">
      ...
      <div className="divide-y divide-gray-100 max-h-80 overflow-y-auto pr-1">
        {cart.map((product: ProductInCart) => (
          <div key={product.pro_id} className="flex items-center gap-4 py-4 first:pt-0 last:pb-4">
            ...
            <div className="flex flex-1 flex-col justify-center min-w-0">
              ...
              <p className="text-xs text-gray-400 mt-0.5">{product.cat_nombre}</p>
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <div className="flex items-center gap-2 bg-gray-50 px-2 py-0.5 rounded-lg border border-gray-200">
                  ...
                  <span className="text-xs font-semibold text-gray-700">{product.count}</span>
                  <button
