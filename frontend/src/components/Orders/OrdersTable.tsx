...
export function OrdersTable() {
  ...
  const allSelected =
    orders.length > 0 && orders.every((o) => selected.has(o.ord_id))
  ...
  const handleExport = async () => {
    setIsExporting(true)
    try {
      await exportOrdersCSV(exportParams, token)
    } finally {
              ...
              orders.map((order: Order) => (
                ...
                >
                  ...
                  <td className='px-3 py-4 text-gray-900'>
                    {formatCurrency(order.ord_total)}
                  </td>
                  <td className='px-3 py-4 text-gray-500'>
                    {formatDate(order.ord_expira_en)}
                  </td>
